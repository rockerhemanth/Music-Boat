...
module.exports = {
    ...
    exec: async (msg, args) => {
        ...
        if ((!music.player || !music.player.playing) && missingPerms.length)
            return msg.channel.send(util.embed().setDescription(`<a:rtest:810143135538741308> | I need ${missingPerms.length > 1 ? "these" : "this"} permission${missingPerms.length > 1 ? "s" : ""} on your voice channel: ${missingPerms.map(x => `\`${x}\``).join(", ")}.`));
        ...
        try {
            ...
            if (music.player && music.player.playing) {
                resultMessage.edit(util.embed()
                    .setAuthor(" |   added to the queue", music.client.user.displayAvatarURL())
