...
async function swap_pages2(client, message, embeds) {
  ...
  .addComponents(
     new MessageButton()
  .setStyle('DANGER')
  .setEmoji('<:double_Left:922144137170550805>')
  .setCustomId('first'),
 new MessageButton()
  .setStyle('DANGER')
  .setEmoji('<:Arrow_Left:922144582320398376>')
  .setCustomId('back'),
  new MessageButton()
  .setStyle(`DANGER`)
  .setEmoji('<:home:922152839873187880>')
  .setCustomId('home'),

  new MessageButton()
  .setStyle(`DANGER`)
  .setEmoji('<:right_Arrow:922144716340998186>')
  .setCustomId('next'),
  new MessageButton()
  .setStyle('DANGER')
  .setEmoji('<:Double_ArrowRight:922143864909869138>')
  .setCustomId('last')
			);
  ...
  if (embeds.length === 1) return message.reply({embeds: [embeds[0]], allowedMentions: { repliedUser: false }})
  const queueEmbed = await message.reply(
    {
      content: `**Current Page - ${currentPage + 1}/${embeds.length}**`,
      components: [buttonrow1],
      embeds: [embeds[currentPage]], allowedMentions: { repliedUser: false }
    }
  );
...
if(interaction.customId == "next"){
  ...
  currentPage = 0;
        queueEmbed.edit({content: `**Current Page - ${currentPage + 1}/${embeds.length}**`, embeds: [embeds[currentPage]], components: [buttonrow1], allowedMentions: { repliedUser: false }});
        queueEmbed.edit({content: `**Current Page - ${currentPage + 1}/${embeds.length}**`, embeds: [embeds[currentPage]], components: [buttonrow1], allowedMentions: { repliedUser: false }});
        queueEmbed.edit({content: `**Current Page - ${currentPage + 1}/${embeds.length}**`, embeds: [embeds[currentPage]], components: [buttonrow1], allowedMentions: { repliedUser: false }});
